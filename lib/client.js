...
function Panel(props) {
  ...
  const refresh = async (force = false) => {
    ...
    setBusy("refresh");
    const toolchain = rpc("toolchain", force ? { refresh: true } : void 0).then((value) => {
      setTc(value);
      setTcMsg("");
    ...
    const emuRead = rpc("emu.list").then((emu) => {
      setEmuRaw(emu.raw || "(\u7A7A)");
      ...
      setEmuRaw(`\u83B7\u53D6\u5931\u8D25:${error.message}`);
      return [];
    });
    const deviceRead = rpc("devices").then((dev) => {
...
${v.output}`);
  ...
  nodes.push(h(
    ...
    }, logs.length === 0 ? "(\u6784\u5EFA/\u90E8\u7F72\u4FE1\u606F\u5C06\u663E\u793A\u5728\u8FD9\u91CC)" : logs.map((l, i) => h("div", { key: i, style: logLine(l.kind) }, l.text)))
  ));
