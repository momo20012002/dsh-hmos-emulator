  ...
  const deploy = async () => {
    ...
    try {
      const res = await fetch(`${API}/deploy`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ projectPath: project, device, module: modules.length > 1 ? moduleSel : void 0 })
      });
      if (res.status !== 200) {
...
${v.output}`);
  ...
  nodes.push(h(
    ...
    h(
      ...
      h(
        "div",
        { style: { ...field, display: "flex", alignItems: "center", color: device ? s.fg : s.faint, cursor: "default" } },
        device ? devInst ? `${devInst.name} (${device})` : device : "\u6682\u65E0\u5728\u7EBF\u8BBE\u5907"
      )
    ),
    device ? h("div", { style: { fontSize: 11, color: s.faint, marginTop: 4 } }, "\u90E8\u7F72\u5C06\u53D1\u9001\u5230\u8BE5\u8BBE\u5907(\u542F\u52A8\u6A21\u62DF\u5668\u540E\u81EA\u52A8\u66F4\u65B0)") : devices.length ? h("div", { style: { fontSize: 11, color: s.faint, marginTop: 4 } }, `\u68C0\u6D4B\u5230 ${devices.length} \u53F0\u5728\u7EBF\u8BBE\u5907,\u5DF2\u9ED8\u8BA4\u9009\u7B2C\u4E00\u53F0`) : h("div", { style: { fontSize: 11, color: s.faint, marginTop: 4 } }, "\u6682\u65E0\u5728\u7EBF\u8BBE\u5907;\u8BF7\u5148\u5728\u300C\u6A21\u62DF\u5668\u5B9E\u4F8B\u300D\u542F\u52A8\u6A21\u62DF\u5668")
  ));
  nodes.push(h(
    "div",
