    ...
    h(
      ...
      open ? h(
        ...
        { style: {
          ...
          right: 0,
          top: "calc(100% + 4px)",
          background: "var(--dsh-bg-solid, #1d1f28)",
          border: `1px solid ${s.border}`,
          borderRadius: 8,
          padding: 4,
...
function Panel(props) {
  ...
  useEffect(() => {
    devRef.current = { device, instanceSel, instances };
  }, [device, instanceSel, instances]);
  useEffect(() => {
    if (!visible) return;
    const id = setInterval(async () => {
      try {
        ...
        setDevices(list);
        const { device: cur, instanceSel: sel, instances: instances2 } = devRef.current;
        const selSerial = sel ? instances2.find((it) => it.name === sel)?.serial : void 0;
...
${value.output}`);
  ...
  nodes.push(h(
    ...
    h(
      ...
      selInst ? h(
        "span",
        { style: { display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, color: selInst.status === "running" ? s.ok : s.faint } },
  ...
  const devInst = device ? instances.find((it) => it.serial === device) : void 0;
  nodes.push(h(
    Card,
    ...
    h(
      ...
      h(Dropdown, {
        value: device,
        placeholder: devices.length ? "\u9009\u62E9\u5728\u7EBF\u8BBE\u5907\u2026" : "(\u65E0\u5728\u7EBF\u8BBE\u5907,\u5148\u542F\u52A8\u6A21\u62DF\u5668)",
        options: devices.map((d) => {
          const inst = instances.find((it) => it.serial === d);
          return { value: d, label: inst ? `${inst.name} (${d})` : d, status: inst ? inst.status : void 0 };
        }),
        onChange: setDevice
      })
    ),
    devices.length ? device ? h("div", { style: { fontSize: 11, color: s.faint, marginTop: 4 } }, `\u5DF2\u9009\u62E9\u76EE\u6807:${devInst ? `${devInst.name} ` : ""}${device}`) : h("div", { style: { fontSize: 11, color: s.faint, marginTop: 4 } }, `\u5728\u7EBF\u8BBE\u5907 ${devices.length} \u53F0,\u8BF7\u9009\u62E9\u90E8\u7F72\u76EE\u6807`) : h("div", { style: { fontSize: 11, color: s.faint, marginTop: 4 } }, "\u6682\u65E0\u5728\u7EBF\u8BBE\u5907;\u8BF7\u5148\u5728\u300C\u6A21\u62DF\u5668\u5B9E\u4F8B\u300D\u542F\u52A8\u6A21\u62DF\u5668,\u518D\u70B9\u300C\u5237\u65B0\u300D")
  ));
  nodes.push(h(
    "div",
