/**
 * dsh-hmos-emulator — 客户端(browser)half。
 *
 * 向 dsh-better-sidebar 注册「鸿蒙模拟器」标签,以产品化 UI 提供:
 *  - 应用工程选择:路径输入框 + 内置文件浏览器(逐级浏览/一键扫描鸿蒙工程);
 *  - 模拟器实例:一键扫描可用模拟器 → 下拉选择 → 启动/停止;
 *  - 部署目标:刷新在线设备并选择;
 *  - 一键“构建并部署”:把选定工程 devecocli run 到目标设备并回显输出。
 *
 * 图标来自 iconfont(assets/icons),内联为 SVG(currentColor 取色);betterSidebar
 * 未安装时由 inject 等待服务,不主动报错。
 * 产物为 DSH 客户端插件官方形态(window.__ModuleLoader__.load …)。
 */
window.__ModuleLoader__.load({
  id: 'dsh-hmos-emulator',
  factory: (require) => {
