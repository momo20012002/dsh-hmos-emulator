...
function capTree(tree) {
  return tree.length > 6e3 ? `${tree.slice(0, 6e3)}
...
function pickLayoutLine(tree, label) {
  ...
  const emuUi = {
    ...
    async execute(args) {
      ...
      if (action === "layout") {
        ...
        const r = await run(["ui", "layout", "--device", device, "--depth", String(depth)], 3e4);
