...
function createApi(config) {
  ...
  api.toolchain = async () => {
    ...
    return {
      ...
      sdkHome: sdk || null,
      hint: "devecocli \u7F3A\u5931\u65F6:\u5B89\u88C5 @deveco/deveco-cli \u6216\u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF DSH_HMOS_DEVECO_CLI\u3002hdc \u7F3A\u5931\u65F6:\u8BBE\u7F6E DEVECO_SDK_HOME(\u5982 C:\\Program Files\\Huawei\\DevEco Studio\\sdk)\u3002"
    };
  };
  api["emu.list"] = async () => {
  ...
  api.devices = async () => {
    ...
    let hdcError = null;
    if (hdc) {
      const result = await runCli([hdc, "list", "targets"], { timeoutMs: 3e4 });
      raw = result.output;
      if (result.code === 0) {
  ...
  api.deploy = async (payload) => {
    ...
    const hdc = resolveHdc();
    if (hdc) {
      const probe = await runCli([hdc, "list", "targets"], { timeoutMs: 15e3 });
