...
function createApi() {
  ...
  api.toolchain = async () => {
    ...
    return {
      ...
      sdkHome: sdk || null,
      hint: `devecocli \u7F3A\u5931\u65F6:\u5B89\u88C5 @deveco/deveco-cli \u6216\u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF DSH_HMOS_DEVECO_CLI\u3002hdc \u7F3A\u5931\u65F6:\u8BBE\u7F6E DEVECO_SDK_HOME(${sdkExample})\u540E\u91CD\u542F dsh web\u3002` + (process.platform === "linux" ? "Linux \u6A21\u62DF\u5668\u6765\u81EA Command Line Tools(\u9700 26.0.0 Release \u53CA\u4EE5\u4E0A):\u628A DEVECO_CLI_CLT_PATH \u6307\u5411\u5176\u5B89\u88C5\u76EE\u5F55\u3002" : "")
    };
  };
...
function pruneAutoShots(dir, keep = 20) {
  try {
    const shots = readdirSync(dir).filter((f) => f.startsWith(AUTO_SHOT_PREFIX) && f.endsWith(".png")).map((f) => ({ f, ms: statSync(join(dir, f)).mtimeMs })).sort((a, b) => b.ms - a.ms);
    let removed = 0;
...
function createToolDefs(api) {
  ...
  const emuUi = {
    ...
    async execute(args, exec) {
      ...
      const dumpLayout = async (full = false) => {
      ...
      const perform = async (a) => {
        ...
        if (action === "layout") {
          ...
          return a?.asJson ? { ok: true, device, total: dump.lines.length, ...page ? { page } : {}, nodes: layoutJson(rows, dump.lines) } : { ok: true, device, total: dump.lines.length, ...page ? { page } : {}, tree: renderFor(rows, a?.changedOnly === true) };
        }
        if (action === "click" || action === "longPress" || action === "doubleTap") {
          ...
          const r = await run(["ui", verb, String(point.x), String(point.y), "--device", device], 2e4);
        ...
        if (action === "swipe") {
          ...
          const r = await run(["ui", "swipe", String(a.x), String(a.y), String(a.x2), String(a.y2), "--device", device], 2e4);
          return { ok: r.code === 0, device, error: r.code === 0 ? "" : tailText(r.output, 4) };
        }
